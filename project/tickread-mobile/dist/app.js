...
async function loadRound() {
    ...
    const pages = await Promise.all(manifest.shards.map(async ({ file }) => (await (await fetch(`${base}/${file}`)).json())));
...
function show(screen) {
    for (const item of [home, game, report])
...
function draw(question, revealed = []) {
    ...
    const y = (v) => height - 24 - ((v - low) / range) * (height - 48);
...
function showReport() {
    ...
    const longest = records.reduce((acc, r) => ({
        run: r.correct ? acc.run + 1 : 0,
        best: Math.max(acc.best, r.correct ? acc.run + 1 : 0),
    }), { run: 0, best: 0 }).best;
    byId("report-title").textContent = `${Math.round((correct / records.length) * 100)}% hit rate`;
    byId("report-streak").textContent = `Longest streak: ${longest}`;
