...
function mount() {
    ...
    function updateHomeStat() {
        ...
        if (lifetime.textContent !== nextText)
            lifetime.textContent = nextText;
    }
    const render = async () => {
        updateHomeStat();
        const records = history();
        byId("m2-summary").textContent = records.length
    ...
    byId("m2-back").addEventListener("click", () => {
        screen.hidden = true;
        home.hidden = false;
        void render();
    });
    ...
    byId("m2-file").addEventListener("change", async () => {
        const input = byId("m2-file");
    ...
    byId("m2-clear").addEventListener("click", () => {
