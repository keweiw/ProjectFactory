...
export interface ChartTheme {
  ...
  volumeUp: string;
  volumeDown: string;
  divider: string;      // the setup/future boundary line
}
...
Candle body width is derived from the available width divided by
`setup.length + future.length`, with a 1 px gap, and a minimum of 1 px so a 60-bar
1-minute chart still renders. When the body would be under 3 px the wick is drawn
at 1 px rather than the usual proportional width.
...
- Once `revealCount > 0`, a vertical divider is drawn at the setup/future boundary,
  and future candles are drawn at reduced opacity so the user can see which bars
  are the answer.
...
- `future` may be empty; the chart then renders setup only regardless of `revealCount`.
...
- With `revealCount === 0`, no drawing operation occurs to the right of the
  setup/future boundary x-coordinate — the future must not be leaked as a faint
  or clipped shape.
